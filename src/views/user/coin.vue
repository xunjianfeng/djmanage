<template>
	<div class="index">
		...
		<div class="container">
			<!-- 添加Coin -->
			<div class="add_coin_container">
				<el-dialog title="添加Coin" :visible.sync="dialogFormVisible" :modal-append-to-body="false">
					<el-form :model="form" label-width="90px" label-position="right">
						<el-form-item label="添加Coins：">
							<el-input v-model.number="form.coin_num" placeholder=""></el-input>
						</el-form-item>
					</el-form>
				...
				<div class="add_integral_container">
					<el-dialog title="是否确定" :visible.sync="confirmVisible" :modal-append-to-body="false">
						...
						<div slot="footer" class="dialog-footer">
							...
							<el-button type="primary" @click="submit_coin">确定</el-button>
						</div>
					</el-dialog>
				</div>
			</div>
			...
			<div class="top_tabs">
				...
				<div class="tabs_block">用户Coin</div>
			</div>
			<div class="play_container">
				<div class="play_head">
					<div class="screen_title">
						<div class="screen_text">用户Coin</div>
					</div>
					<!-- 筛选条件 -->
					...
					<div class="play_form">
						<div class="play_table">
							<div class="table_header">
								...
								<div class="table_head_div">Coin总额</div>
							...
							<div class="table_body">
								...
								<div class="body_row" v-for="(item,index) in list" :key="index" v-if="list != ''">
									...
									<div class="row_td_block">{{item.coin}}</div>
									<div class="row_td_block2">
										<div class="see_detail" @click="jump_coin(item.userId)">积分明细</div>
									</div>
									<div class="row_td_block2">
										<div class="see_detail" @click="add_coin(item.userId)">添加Coin</div>
									</div>
								</div>
							</div>
						</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			jump_coin(userId){
				this.$router.push({
					path: '/user/coin_details',query:{userId}
				})
			},
			// coin列表
			getList() {
			...
			confirm_status() {
				for (let i = 0; i < this.list.length; i++) {
					if (this.list[i].userId == this.comfirm_id) {
						// console.log("选到了"+this.comfirm_id)
						if (this.form.coin_num == '') {
							this.$message({
								type: 'warning',
								message: '请输入具体数额！'
							});
						} else if (!(/^[1-9]\d*$/.test(this.form.coin_num))) {
							this.$message({
								type: 'warning',
			...
			submit_coin() {
				let params = {
					userId: this.comfirm_id,
					coin: this.form.coin_num
				}
				addCoins(params).then(res => {
					this.$message({
						type: 'success',
						message: res.message
					});
